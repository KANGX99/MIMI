...
IMG_EXTENSIONS = ['.jpg', '.jpeg', '.png', '.ppm', '.bmp', '.pgm', '.tif', '.BMP']
...
class ImageDataset2(Dataset):
    def __init__(self, csv_file,
        ...
        self.test = test
        if not self.test:
            mos = self.data.iloc[:,1]
            self.max_mos = max(mos)
            self.min_mos = min(mos)
            self.data.iloc[:,1] = (mos - self.min_mos)/(self.max_mos - self.min_mos) *10
